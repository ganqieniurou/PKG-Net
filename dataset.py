...
def get_dataset(dataset_name, dir, mode='train', context_frame_num=0, border_mode='hard',
                all_bboxes=None, patch_size=32, of_dataset=False):
    if not of_dataset:
        img_ext = {"ped2": ".tif", "avenue": ".jpg", "shtech": ".jpg"}[dataset_name]
    else:
        img_ext = ".npy"
...
class Chunked_sample_dataset(Dataset):
    ...
    def __getitem__(self, indice):
        ...
        return self.transform(x), indice


if __name__ == '__main__':
    root = 'D:/Expproject/dataset/ped2'
    #dataset = ped_dataset(root)
    dataset = get_dataset('ped2', root)
    print(len(dataset))
